...
static void fillWaveTable(int n, Z* amps, int ampStride, Z* phases, int phaseStride, Z smooth, Z* table)
{
	...
	// Use heap allocation to avoid stack overflow (384KB total)
	std::vector<Z> real(kWaveTableSize2, 0.0);
	std::vector<Z> imag(kWaveTableSize2, 0.0);
	std::vector<Z> polar(kWaveTableSize);
	std::vector<Z> rect(kWaveTableSize);
	...
	vDSP_ztocD(&in, 1, (DSPDoubleComplex*)polar.data(), 1, kWaveTableSize2);
	vDSP_rectD(polar.data(), 1, rect.data(), 1, kWaveTableSize2);
	vDSP_ctozD((DSPDoubleComplex*)rect.data(), 1, &in, 1, kWaveTableSize2);
	rifft(kWaveTableSize, real.data(), imag.data(), table);
}
