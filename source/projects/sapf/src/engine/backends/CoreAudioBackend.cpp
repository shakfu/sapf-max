...
static void playWithAudioUnit(Thread& th, V& v)
{
...
static void recordWithAudioUnit(Thread& th, V& v, Arg filename)
{
	if (!v.isList()) wrongType("play : s", "List", v);
...
{
public:
	void play(Thread& th, V& v) override { playWithAudioUnit(th, v); }
	void record(Thread& th, V& v, Arg filename) override { recordWithAudioUnit(th, v, filename); }
	void stopAll() override { stopPlaying(); }
	void stopFinished() override { stopPlayingIfDone(); }
};

} // namespace

std::unique_ptr<AudioBackend> CreateCoreAudioBackend()
{
	return std::unique_ptr<AudioBackend>(new CoreAudioBackend());
}

bool SupportsCoreAudioBackend()
{
	return true;
}
